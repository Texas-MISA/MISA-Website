...
- 🪤 **A marquee needs enough copies to cover the VIEWPORT, and "duplicate twice, translate ‑50%" only does that when one group is wider than the screen.** Neither home-page track is: measured at a 1646px viewport, the groups are 1360px and 1272px, so each cycle ended with ~300px of bare navy and a snap. **The translate distance is one group width in pixels** (`--marquee-shift`, computed by the component), never a percentage — a percentage silently couples the distance to the copy count. Copies come from `Math.ceil(MAX_VIEWPORT / groupWidth) + 1`; `MAX_VIEWPORT` (4000) is a **real ceiling**, not a margin, and is documented at its definition. 📌 No pause on hover: the tracks are full-width, so a resting mouse froze a row and read as breakage.
