...
| [`docs/design/`](docs/design/) | 🧰 **The design toolkit's records (2026-09-18).** `surfaces.json` is the registry the brief-guard hook, `tests/design-receipts.test.ts` and `tests/ui` all read; `surfaces/<surface>/receipts/` holds each step's receipt; `templates/` the blanks. The BRIEF lives at `.impeccable/surfaces/<slug>.md` — impeccable's own brief, so the lead loads it. 🔓 **A registered surface cannot be edited until its brief exists** — the hook refuses, and `MISA_DESIGN_GUARD=off` at launch is the human-only bypass. Start a redesign with `/design-brief <surface>`; review it with `/design-gate <surface>`. ✅ **TWO of the four portal surfaces are `rebuilt`: `portal-hub` (2026-09-19) and `portal-attend` (2026-09-20)**, seven receipts each plus an `officer.md` — 🔴 **the officer removed the navy header from every portal page on 2026-09-20**, which edits both frozen surfaces, and an `officer.md` naming that fix commit is the ONLY thing keeping the checker from failing them as stale; the other two are `in-progress` (brief, two concepts and evidence each, with an officer-adopted concept), their files unguarded, their builds next. 🔒 **A `rebuilt` surface's files are FROZEN:** `receipts.mjs` fails it as **stale** on any later commit touching them that no receipt names as a `fix_commit`. `components/ui/` is exempt by design. 🔴 Never `/impeccable init`, `/impeccable document` onto DESIGN.md (its sidecar-only refresh is fine), impeccable's replace-DESIGN.md path, or `ui-ux-pro-max --persist`: each writes a rival source of truth. |
...
| [`docs/handoff-portal-rebuild-rounds-4-7.md`](docs/handoff-portal-rebuild-rounds-4-7.md) | 📋 **THE CURRENT EXECUTION PROMPT for finishing phase 3 (parts 4–7), written 2026-09-20** after the re-gate rounds. Copy it into a fresh session. It opens with **🔴 CONTEXT DISCIPLINE** — the rules that let an orchestrator run four rounds without running out of room, which is what ended the session that built parts 0–3 — then carries the verified state, the four round scopes, the **five defect classes rounds 1–3 uncovered**, and the **six stacked officer questions**. |
| [`docs/handoff-finish-portal-rebuild.md`](docs/handoff-finish-portal-rebuild.md) | 🗑️ **SUPERSEDED by the row above.** Its rounds 1–3 are done and several of its numbers were re-derived and moved (`607.7` is the desktop-window reading; the hub's `265.5` is `267.2`). **Kept for the officer’s full DELETE/KEEP copy lists** and the three earlier approvals they supersede, which the new prompt cites rather than repeats. |
| [`docs/frontend-redesign-v2-plan.md`](docs/frontend-redesign-v2-plan.md) | 🏗️ **THE CURRENT PLAN, PART-BUILT.** ✅ Phases 0–2 and **4** are **COMPLETE AND LIVE** (4 taken out of order, officer's call; merged 2026-08-31). 🏗️ **Phase 3 — 📛 the PORTAL REBUILD — is IN PROGRESS: the UI redesign of the member portal and every page in it** (`/portal`, `/portal/attend`, `/portal/leaderboard`, `/portal/lookup`). ✅ **Read its *Phase 3 brief* section first, starting with 🧭 *Where the build stands*:** it carries the four briefs' locations, each surface's adopted concept and measured gate bar, the copy the officer approved, the per-part state with commits, and the open officer questions. 🏗️ **Parts 0–3 are done, TWO surfaces are `rebuilt` — `portal-hub` and `portal-attend` — and BOTH WERE RE-GATED against the sheet build on 2026-09-20 (rounds 2 and 3), because their original receipts had reviewed a build with a navy band that no longer exists. 🔴 An officer receipt cures staleness; it does not constitute a review.** Check-in's bar is met under both scrollbar conventions (**591.5px at a true 360×640**, 611.5/607.66 in a 360px desktop window); the hub's is **267.2px** against ≤424. All twelve check-in states gated, `test:ui` 39/3 → 41/1. **Part 4, `/portal/lookup`, is next** — its handoff prompt is [`docs/handoff-portal-rebuild-rounds-4-7.md`](docs/handoff-portal-rebuild-rounds-4-7.md) — it carries nine of the eleven remaining muted occurrences and the one red `test:ui` check. 🧰 Run through the design toolkit (`DESIGN.md` §Design toolkit: `/design-brief` per surface — done — then build, then `/design-gate`), and it **owns the `--misa-muted`-on-Vellum AA fix, folded into it by the officer on 2026-09-01 rather than patched first — so that failure stays live until phase 3 ships** — it is now the only phase left before 5 (reconcile the rest of `docs/invariants.md`). |
| [`docs/member-portal-plan.md`](docs/member-portal-plan.md) | 🏗️ **PHASE 1 BUILT on `portal-phase-1` (2026-09-18), verified locally, AWAITING MERGE.** A pure move of `/attend`, `/leaderboard`, `/lookup` to `/portal/*` plus a static hub at `/portal`; the old paths are 🔓 **permanent redirects in `next.config.ts`, never deleted** (`tests/portal.test.ts`). The site's one way in is the header's navy MEMBER PORTAL button, and check-in lives only inside the portal (officer). ⏭️ **Next: the portal's UI redesign (v2 phase 3), through the design toolkit** (`design-toolkit`, stacked on this branch) — ✅ all four surfaces briefed 2026-09-19; `portal-hub` and `portal-attend` are built and `rebuilt`, `/portal/lookup` and `/portal/leaderboard` remain. No redesign, no migration; v2 phase 3 stays separate. `/admin` stays separate; houses/bingo (phases 2–4) ON HOLD — officer-edited, members view via server-side polling. Rollback tag `pre-portal-2026-09-18`; revert the portal commits, not the merge. |
...
- **Every shared UI primitive lives in `components/ui/`.** Reaching for a class string instead is how drift accumulates. 🔴 **And a primitive with no call sites has not ended the drift it was written to end** — v2 phase 4 found `PageHeader`, `SectionHeading` and (in `/admin`) `Table` at **zero call sites**, each authored against duplication that was still there, verbatim, in 25 pages and 11 tables. Extracting the component is half the job.
- 🪤 **`<Section>` is PUBLIC-ONLY and has zero admin call sites by design.** It owns the public gutter and vertical rhythm, which `/admin` does not share; `Panel` is the officer surface and the shell owns the ground. 🪤 `Panel` does **not** forward `action`, so a `<form>` that needs one stays a `<form>` with a `bg-white` frame.
- 🔴 **Moving a page ground is a CONTRAST CHANGE, not a paint change.** Five shared primitives fill with `bg-misa-panel` (`controlClass`, `table.tsx`'s sticky `<THead>`, `chip.tsx`'s `FilterChip`, `banner.tsx`'s neutral variant, `Tr`'s hover), and `--misa-muted` is 4.33:1 on that grey and fails AA. **Wrap the screens in white surfaces first and flip the ground last** — the reverse order looks finished and is measurably broken. Re-measure every grey sitting on any ground you move.
