...
**Stages 0–5 complete; Stage 6 (member directory) in progress — phases 1, 2a, 2, 3, and 4 of 9 built, merged, and deployed (phase 4 on 2026-08-05, doc v1.34). Phase 5 (selection and extraction) is next.** Stage 6 merges to `main` at the end of each phase rather than at the end of the stage, as Stage 5 did.
...
- 🔓 **A CSV cell beginning `=`, `+`, `-`, or `@` executes as a formula when an officer opens the file, and member names are attacker-supplied** — anyone who can check in during an open window picks their own (§6). Escape it in the CSV writer; the victim is the officer's machine, not the server. The `.xlsx` path is *not* exposed, since its cells are written as typed values — which is why the two writers must not share a "join it with commas" shortcut, and why xlsx must never be implemented as CSV with a different extension. (Stage 6 phase 5, not built.)
...
lib/member-fields.ts    fetchFieldDefinitions — the live custom-field
                        ...
                        bound that matters
lib/export.ts           Stage 6 phase 5, not built — CSV / TSV / clipboard, the
                        exportable field catalogue, and the .xlsx workbook
                        builder (doc v1.30). Pure: rows + chosen fields in,
                        string or bytes out. The audit write and the response
                        headers belong to the route handler, not here
app/admin/(shell)/members/export/route.ts
                        Stage 6 phase 5, not built — the CSV/.xlsx download.
                        The first Route Handler in the codebase, because a
                        download needs Content-Disposition and a Server Action
