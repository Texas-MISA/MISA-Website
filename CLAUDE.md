...
- **Nothing is ever dropped on the floor.** A check-in matching no open event and/or no roster member is still stored, as `pending`, for an officer to resolve. A member who showed up and got no credit is the failure mode that erodes trust in the whole system. (§4.2)
  - ⚠️ **Narrowing decided 2026-07-31; do not apply until the code lands.** The `/attend` first-time-checkbox change makes this cover only submissions that resolve to a **known member**. A submission matching no roster member whose submitter did *not* tick "first time" will be **discarded rather than stored** — re-prompted instead, however many times it fails — because the alternative is silently creating a duplicate member for every double typo, and duplicates have no merge tool (see the Stage 6 notes). The accepted cost is that someone who never gets their details right leaves no trace at all; the recovery path becomes officer manual entry at `/admin/attendance/new`, which officers have to be told about. **The rule above is still literally true of the code today**, so amend it in the same commit that changes `lib/checkin.ts` — never before, or the doc starts lying in the other direction. Full spec, decision table, and the accepted membership-oracle tradeoff: [`docs/attend-confirmation-flow.md`](docs/attend-confirmation-flow.md).
