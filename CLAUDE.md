...
| [`docs/frontend-redesign-v2-plan.md`](docs/frontend-redesign-v2-plan.md) | 🏗️ **THE CURRENT PLAN, PART-BUILT.** ✅ Phases 0–2 and **4** are **COMPLETE AND LIVE** (4 taken out of order, officer's call; merged 2026-08-31). ⏭️ **Phase 3 is the NEXT TASK (officer, 2026-09-18): the UI redesign of the member portal and every page in it** (`/portal`, `/portal/attend`, `/portal/leaderboard`, `/portal/lookup`), after portal phase 1 merges, 🧰 **run through the design toolkit** (`DESIGN.md` §Design toolkit, built on `design-toolkit`: `/design-brief` per surface, then `/design-gate`), and it **owns the `--misa-muted`-on-Vellum AA fix, folded into it by the officer on 2026-09-01 rather than patched first — so that failure stays live until phase 3 ships** — it is now the only phase left before 5 (reconcile the rest of `docs/invariants.md`). |
| [`docs/member-portal-plan.md`](docs/member-portal-plan.md) | 🏗️ **PHASE 1 BUILT on `portal-phase-1` (2026-09-18), verified locally, AWAITING MERGE.** A pure move of `/attend`, `/leaderboard`, `/lookup` to `/portal/*` plus a static hub at `/portal`; the old paths are 🔓 **permanent redirects in `next.config.ts`, never deleted** (`tests/portal.test.ts`). The site's one way in is the header's navy MEMBER PORTAL button, and check-in lives only inside the portal (officer). ⏭️ **Next: the portal's UI redesign (v2 phase 3), through the design toolkit** (`design-toolkit`, stacked on this branch). No redesign, no migration; v2 phase 3 stays separate. `/admin` stays separate; houses/bingo (phases 2–4) ON HOLD — officer-edited, members view via server-side polling. Rollback tag `pre-portal-2026-09-18`; revert the portal commits, not the merge. |
