...
**Stages 0–5 complete; Stage 6 (member directory) in progress — phase 1 of 6 built, merged, and deployed 2026-08-01 (doc v1.25).** Stage 5's four build phases were each merged to `main` as they finished rather than at the end of the stage, and the phase-5 read-through closed it (v1.24); Stage 6 follows the same rhythm. Stage 4 added the officer admin section: sign-in at `/admin/login`, the schedule at `/admin/events` (create, edit, duplicate, recurring series, lifecycle), with every mutation writing an `admin_audit` row.

Stage 6 phase 1: **migration 14** (`member_directory.attendance_rate`, `members.notes`, `'roster'` as an `admin_audit` entity type — **applied to local and remote**), `lib/filters.ts`, and the read-only directory at `/admin/members` with server-side sorting, pagination, and the filters that live on the view. ⚠️ **It has not been walked through a browser yet** — that is the first item of phase 2. `tasks.md` has the six-phase plan and the traps.
...
lib/export.ts           Stage 6 phase 2, not built — CSV / TSV / clipboard
