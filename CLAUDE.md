...
**Stages 0–5 complete; Stage 6 (member directory) in progress — phases 1, 2a, 2, 3, 4 and 5 of 9 built and browser-verified (phase 5 on 2026-08-06, doc v1.37). Stage 6.5 (dues) interrupts here, before phase 6.** Stage 6 merges to `main` at the end of each phase rather than at the end of the stage, as Stage 5 did.
...
- **Dues status is calculated, never ticked** (Stage 6.5, planned — doc v1.34, spec `docs/dues-and-membership.md`). "Official member" means a non-voided `dues_payments` row whose `covered_terms` includes `current_term()`, surfaced as the single boolean `member_directory.dues_paid_current_term` and rendered **Paid / Not Paid**. It gates nothing (§9 #12): check-in, points, and the leaderboard are untouched, which is the reversible choice — a gate later is application logic over a column that already exists.
  ...
  - 🪤 **Terms do not sort lexicographically.** `'Fall 2026' < 'Spring 2026'` is true as a string compare and false as a calendar fact. Every "which term is later" question goes through `terms_from` / `termsFrom` ordering — never `max(term)`, never `order by term`. Same class of error as `new Date("2026-09-01T18:00")`: plausible output, wrong answer, no error anywhere. This is why there is no "paid through" column on the view.
...
lib/dues.ts             Stage 6.5, not built — the dues domain core, pure like
