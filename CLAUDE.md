...
- **Match and dedupe on the normalized generated column, never the raw submitted value.** (§4.2)
  - **Amended 2026-08-01, and not yet applied — Stage 6 phase 2 does the rename.** The columns become `members.normalized_eid` and `attendance.normalized_eid`, generated from `eid` / `submitted_eid`, and the identifier itself becomes a real UT EID (alphanumeric, `abc1234`) rather than `UT` + six sequential digits. Until that migration lands the columns are still `normalized_student_id` / `submitted_student_id` — check the schema rather than this line.
  - **What the normalization buys changes with it.** The old worked example (`ut-12345`, `UT 12345`, `UT12345` are one person) exercised whitespace and hyphen stripping, which EIDs never contain. After the rename the only thing it buys is case-insensitivity, and the fold goes to **lower**, not upper, because EIDs are conventionally lowercase and the review screen otherwise shouts `ABC1234` back at someone who typed `abc1234`. The stripping stays in the expression anyway — harmless, and removing it would rewrite every stored value for no gain.
  - **The suggestion ranker's calibration does not survive the format change.** `id_contains` exists solely to catch a dropped `UT` prefix and becomes a broad substring match on short alphanumerics; distance-1-stands-alone was tuned for 8-char strings with a constant 2-char prefix. Both are re-derived in phase 2, empirically, against a regenerated seed — the current constants came from looking at a real review screen offering three strangers, not from reasoning, and their replacements must too.
