...
🔴 **Production is not purely fabricated data.** It carries one **real member** (`Christian A Gonzales / cag7284`, self-registered through the live form) alongside a real officer account, which is why `seed.sql`'s guard refuses to run against the remote — correctly. Never work around that guard; use a targeted migration, as 17 does. Production totals (33 members / 16 events / 209 attendance / 12 audit) are not the seed's and are not meant to be.
...
- 🔓 **A custom-field key is interpolated into an `order=` term, so its format check is a security control, not a naming convention.** A directory sort on an officer-defined field becomes `custom_fields->>key`, and PostgREST parses whatever comes out. The phase-4 spike (2026-08-02) measured what an unconstrained key buys: a key containing `,` **breaks out of the order term and is read as a second order column** — `custom_fields->>a,b` came back `42703 column … b does not exist`, meaning an arbitrary column name would have been accepted. `.` and `)` fail as PGRST100 parse errors, but a **space and a `"` were accepted silently, with no error at all**, so "PostgREST rejected it when I tried" is not a boundary. `FIELD_KEY_PATTERN` (`^[a-z][a-z0-9_]{0,39}$`) in `lib/members.ts` is the escape, enforced three times on purpose: the zod schema, `member_field_definitions_key_format` in migration 18, and again in `customSortColumn()` — which is the one place a key becomes part of a query string and therefore must re-check rather than trust its caller. Ordering by a hidden column leaks no values; that is not a reason to leave the door open.
...
- **A paginated list needs a total order, not merely a sort.** Rows tied on the sort column can come back arranged differently per request, so pages skip and repeat rows — and it reads as missing data rather than as an ordering fault. Every directory query ends `.order("id")`. The seeded roster is 32 members and cannot show this: fixtures have to exceed `PAGE_SIZE` deliberately.
  - **Custom-field sorts make this the common case rather than the edge case.** A dropdown has a handful of options across the whole roster, so almost every row is tied. The spike reproduced the failure directly: 30 rows paged at 7 came back as 30 rows but 29 distinct — one duplicated, one never shown. `tests/member-directory.test.ts` pins it against real PostgREST, on its own `t3qcf` fixtures rather than the file's shared `t3q` ones, because earlier blocks add members as they run.
...
lib/filters.ts          directory filter core: parse → MemberFilter → query.
                        ...
                        MemberFilter is built, so no caller needs a second escape
...
app/admin/(shell)/members/_components/selection.tsx
                        ...
                        that happens to be full. export-toolbar.tsx holds the
