...
- **Both views are scoped to `app_settings.current_term`.** Any new aggregate must use the same scope, denominators included — an all-time `events_possible` against a current-term `events_attended` understates every rate and still looks plausible. (§4.4, §4.5)
