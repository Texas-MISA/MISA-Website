...
Stage 6 phase 2: **migration 16** (the EID rename — columns, constraints, indexes, the `lower()` fold, and `member_directory` dropped/recreated with the anon revoke re-issued) and **migration 17** (an idempotent backfill of the remote's pre-rename values; 18 migration files total, local and remote identical). The ranker lost `id_contains`; the distance-2 floor stays for a new reason. `seed.sql` regenerated with EID-shaped identifiers and now asserts its own counts. The attendance queue no longer shows a Member column — the submission detail page still shows and edits it.
...
lib/members.ts          member domain core: classifyTermEvents (the detail page's
                        three-state grid) and formatAttendanceRate. Phase 4 adds
                        custom-field definitions, option validation, and
                        AUDITED_MEMBER_COLUMNS to the same file
