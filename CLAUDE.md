...
| [`docs/frontend-redesign-v2-plan.md`](docs/frontend-redesign-v2-plan.md) | 🏗️ **THE CURRENT PLAN, PART-BUILT.** ✅ Phases 0–2 and **4** complete (4 taken out of order, officer's call). ⏸️ **Phase 3 is DEFERRED, not skipped** (`/attend`, `/leaderboard`, `/lookup`) and still owes the `--misa-muted`-on-Vellum AA fix. Then 5 (reconcile the rest of `docs/invariants.md`). |
| [`docs/frontend-redesign-plan.md`](docs/frontend-redesign-plan.md) | 📋 **SUPERSEDED by v2 above**, kept for reasoning. |

## Repository status

**Stages 0–8 are COMPLETE. Stage 9 (launch) is next.** 30 migration files, through `…000029`. ⚠️ **Local is AHEAD of the remote, by TWO now**: migrations 28 (`checkin_origin`) and 29 (`member_directory_terms`) are applied locally and **not yet pushed**, and the code that reads them is not deployed either, so the two are in step — push them together. The next unclaimed number is **30**. 🪤 **26 and 27 were written by two sessions at once and both claimed 26 for a few minutes** — the collision surfaced as a 23505 on `schema_migrations_pkey` during `db reset`.
