...
| [`docs/member-portal-plan.md`](docs/member-portal-plan.md) | 📋 **NOT BUILT (2026-09-18).** Phase 1 (current goal) is a pure move of `/attend`, `/leaderboard`, `/lookup` under `/portal` with permanent 308 redirects — no redesign, no migration; v2 phase 3 stays separate. `/admin` stays separate; houses/bingo (phases 2–4) ON HOLD. House bingo is officer-edited, members view it via server-side polling. Rollback tag `pre-portal-2026-09-18`. |
