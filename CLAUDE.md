...
**Stages 0–5 complete; Stage 6 (member directory) in progress — phases 1, 2a, 2, 3 and 4 of 9 built, merged and deployed (phase 4 on 2026-08-05), and phase 5a built and browser-verified 2026-08-06 (doc v1.36). Phase 5b (the xlsx workbook) is next.** Stage 6 merges to `main` at the end of each phase rather than at the end of the stage, as Stage 5 did.

📦 **Phase 5 was split into 5a and 5b** (2026-08-06), mirroring the mid-stage 2a/2 split. **5a shipped selection and extraction over CSV**: `lib/export.ts` (the pure core — the field catalogue, `projectRow`, the CSV/TSV writers), row checkboxes with a real "select all N matching this filter", a field picker, clipboard actions, and **`app/admin/(shell)/members/export/route.ts`, the codebase's first Route Handler**. **5b is the `.xlsx` workbook and is unbuilt** — ✅ the writer is decided: **hand-rolled, zero dependencies**, because SheetJS's npm package is four years stale (releases moved to its own CDN) and `exceljs` has been inactive since Oct 2023. An xlsx is a zip of ~6 XML parts and `node:zlib` is built in. Reasoning and rejected candidates are in `tasks.md` under Phase 5b.
...
lib/export.ts           the export core (phase 5a) — the field catalogue, the
                        ...
                        response headers belong to the route, not here. The
                        .xlsx builder joins it in 5b as lib/xlsx.ts
