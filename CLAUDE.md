...
| [`DESIGN.md`](DESIGN.md) | 🎨 **THE DESIGN SOURCE OF TRUTH, site-wide, and it is v2 as of 2026-08-19.** Written from what phase 1 actually shipped, not from what was planned: the five grounds, four elevation steps, two radii, the `.plate`/`.sheet`/hatch surfaces, the type ramp, spacing scale, reveal variants, layout-family budget and photography pipeline. ⚠️ It marks which surfaces are **NOT YET REBUILT** — most of them. |
...
| [`docs/checkin-location-verification.md`](docs/checkin-location-verification.md) | ✅ **BUILT — migration 28, 2026-08-22.** Flags check-ins whose network origin differs from the event's modal one. Stores a peppered, per-event digest and **never an IP address**; advisory only, and per-event toggleable (`events.verify_origin`, default true) **after the fact**, because capture is unconditional and only derivation is gated. Its *What this cannot catch* section is still the part to read — the mechanism misses the likeliest form of the fraud, and **cellular is a documented public bypass**. ✂️ §6's rotating venue code was DROPPED in favour of it. Three plan assumptions were wrong when measured and are marked 🔬 in place. |
...
📋 **Spec and threat model: [`docs/checkin-location-verification.md`](docs/checkin-location-verification.md).** Read *What this cannot catch* before trusting a flag.

- 📌 **ADVISORY ONLY, and every part of the design depends on it.** It rejects no check-in, moves no row to `pending`, withholds no points and writes no `admin_audit` row by itself. A flag is *worth a glance at a list*, never evidence about a person — and if it is ever going to be quoted at a member, it should not ship. The pill tones stop at `caution`; nothing here is ever `critical`.
- 🔓 **`event_id` is INSIDE the hash, and being unjoinable across events is the feature.** `sha256(PEPPER || event_id || normalize(ip))` — one address at two events gives two unrelated digests, so the table cannot trace where a member was over a semester. Never "optimise" the event id out to make digests joinable. That property is the whole reason storing this is acceptable.
- 🔓 **`CHECKIN_ORIGIN_PEPPER` is a server-only env secret, never a repo literal.** IPv4 is 4.3 billion addresses; an unpeppered SHA-256 of one is reversible on a laptop, and this repo is public. Missing is **safe, not broken**: nothing is hashed, every row reads *origin unknown*, no check-in fails. `lib/request-ip.ts`'s known scope string is fine for a ten-minute bucket and is **not** fine here.
- 🔓 **The toggle gates DERIVATION, NOT COLLECTION.** `events.verify_origin` (default true) controls whether the review screen derives flags; capture runs on every self check-in regardless. That asymmetry is exactly what lets an officer turn verification on a week after an event and see flags appear with no backfill — and it is why `/attend` carries a disclosure sentence in the unconditional present tense.
- 🪤 **NEVER capture for `source = 'admin_manual'`.** Officer entry runs from the officer's laptop; ten walk-ins would write ten rows on the officer's own origin, which then either *becomes* the venue mode or gets the whole batch flagged. It holds **structurally** — `resolveCheckin` has exactly one caller and the officer path never reaches it — so keep it that way rather than adding a filter.
- 🪤 **The insert fails open.** An advisory signal in the one unauthenticated write path must never turn away a member standing in a room. Same doctrine as `checkRateLimit`.
- 🪤 **A missing address yields `unknown` and NO digest — never a hash of a placeholder string.** In local dev nobody has the header, so a shared literal would give every check-in one digest and form a confident, entirely fictional venue mode.
- 🔓 **`network_type = 'unknown'` implies `origin_hash is null`, enforced by a CHECK.** The review screen's last rule is *"matched no venue and is neither campus nor cellular → off-network, flagged"*, so an unclassifiable address carrying a digest would be reported as evidence on the strength of a parse failure. `checkin_origin_unknown_has_no_digest` makes that a property of the database, like `present_requires_resolution`. **Not an equivalence** — a pending orphan has a real kind and a null digest.
- 🪤 **An IPv4-mapped address is detected on the PARSED VALUE, never by matching text.** `::ffff:a.b.c.d` and `0:0:0:0:0:ffff:a.b.c.d` are the same address; a regex for the canonical spelling folds one and not the other, so one client gets two digests and drops out of its own venue mode. The test is `v6 >> 32n === 0xffffn`. **Structure covers every spelling; a regex covers the one you thought of.**
- 🪤 **The origin insert is wrapped in its own try/catch, and that is the load-bearing half.** It runs AFTER the attendance row is written, so a rejection escaping into `submitCheckin`'s house try/catch returns `error` to a member whose check-in SUCCEEDED — who retries and is told they are a duplicate. PostgREST failures normally arrive as `{ error }`, which is why the rejection path is the one that ships unnoticed.
- 🪤 **The review screen reads origins by EVENT through the embedded attendance row, never an `.in()` list of attendance ids.** One uuid per check-in is ~37 characters; §2.2's worst case of 150 attendees is a 5.5KB query string on a GET — the same shape as the 1,257-character URL that made the member picker abandon id enumeration.
- 🪤 **Window membership goes through `withinCheckinWindow`, not a `>=` on the raw strings.** PostgREST renders `submitted_at` with microseconds and a whole-second `starts_at` without any, so a string compare compares two shapes and is correct only because `.` sorts after `+` in ASCII. Nothing declares that, and a non-UTC offset would break it silently.
- ⚠️ **An officer-entered row has NO origin by design, and must render `not_applicable`, never `unknown`.** `unknown` claims the system tried to determine an origin and could not; for an `admin_manual` row the concept does not apply. The source check sits above the missing-record check in `deriveOriginFlag` for exactly this reason.
- 🔓 **`classifyNetwork` returns `other` only when matching nothing MEANS something.** `other` is the sole flagged label, so returning it asserts the address is confidently not the university's — available only when that address family has campus data to have missed. UT's AS18 announces **zero IPv6**, so an unmatched IPv6 address is `unknown`, never `other`. Self-corrects the moment `CAMPUS_V6` has entries.
- ⚠️ **Cellular is NEVER flagged** (officer's decision, 2026-08-22), and it outranks both flagged states. It removes the largest false-positive class — real attendees whose phone had not joined the wifi — and it is a **documented, public bypass**: turn off wifi and never get flagged. Correct only while this is advisory. **If it ever gates anything, reopen that rule first.**
- 🪤 **Cellular leaves both the mode's groups AND its denominator.** Out of the groups so a tethered hotspot never becomes the venue; out of the denominator so a heavily-cellular event does not dilute its own share below quorum and lose a venue it had.
- **Quorum, or there is no venue.** `VENUE_MIN_COUNT` (5) and `VENUE_MIN_SHARE` (0.5) are **room judgements, not security tuning** — the `RATE_LIMIT_MAX` category. Below either, nothing is flagged rather than everything. **A tie is not a winner**: two origins at the same top count both clear a 0.5 share, so the answer would depend on iteration order.
- ⚠️ **`lib/network-prefixes.generated.ts` is GENERATED — do not hand-edit.** `node scripts/build-network-table.mjs` rebuilds it from RIPEstat. Committed rather than fetched, so there is no runtime dependency and **no request-time network call**: sending members' IPs to a third-party data broker to power an advisory pill inverts the whole design. 🪤 `GENERATED_AT` is the only staleness signal, and staleness fails toward *flagging real attendees*.
...
- ✅ **`DESIGN.md` IS THE DESIGN SOURCE OF TRUTH AGAIN, and it is now v2** (2026-08-19). It was retired for the redesign and rewritten early — phase 5 on paper, moved up because **phase 2 rebuilds five pages and cannot be executed consistently against an undocumented system.** It records what phase 1 actually shipped, read off the running application: the five grounds, the four elevation steps, the two radii, the `.plate`/`.sheet`/hatch surfaces, the type ramp, the spacing scale, the reveal variants and their traps, the layout-family budget, and the photography pipeline. ⚠️ It also marks which surfaces are **NOT YET REBUILT** — `/about`, `/projects`, `/gallery`, `/officers`, `/contact` (phase 2), the member pages (phase 3) and `/admin` (phase 4) still render v1 inside v2 grounds. 🪤 **Do not read "it has the grey background" as "it has been done."** The v1 file is kept verbatim at [`docs/design-v1-superseded.md`](docs/design-v1-superseded.md) for its reasoning; where the two disagree, `DESIGN.md` wins. What phase 5 still owes is `docs/invariants.md`, which is unreconciled and still describes v1 for most surfaces.
- 📌 **The design handoff (`docs/Texas MISA website UI mockups/design_handoff_misa_website/`) is historical reference.** It is desktop-only with no breakpoints authored, defines no focus, hover, empty or error states, and draws five of the site's twenty-odd screens. The identity it set is unchanged and is recorded in `DESIGN.md`; its README remains the only home of the duotone image-treatment spec. Never port its inline styles.
...
- 🔓 **THE NO-PHOTOGRAPHY RULE IS LIFTED, CONDITIONALLY** (2026-08-18/19). Its premise was factual, not aesthetic — the organization had not taken the photographs — and the fact changed. Real photographs are **committed and live** on the home page, `/about`, `/contact`, `/gallery` and — since 2026-08-23 — the project cells on the home page and `/projects`. 🔴 **The repository is public and a face in its history cannot be taken back by a later commit**, which is the part that still binds. What replaces the rule is narrower: **a slot renders a photograph or a labelled `<Hatch>`, never a hole**, and `components/ui/photo-slot.tsx` is the single place that swap happens. 🔓 **Officer headshots landed 2026-08-23, for ALL THIRTEEN, and the rule was SATISFIED rather than waived.** The objection was never "no faces on officer cards" — it was that the handoff shipped headshots while recording that the photo-to-name pairing was never supplied. The officer supplied it, from the live site's own officers page; `lib/officers.ts` documents that the pairing was read off that page's **CSS grid geometry** rather than its DOM order, because a Squarespace fluid-engine grid orders blocks by position and not by document order. 🔴 **ONE card still renders `<Hatch>` and that is deliberate**: the source page showed ONE image file on both Daniel Chen's card and Sanya Pillai's and nothing attributed it, so both waited. The officer attributed it to Daniel Chen on 2026-08-23 by renaming the file in the saved export — which is exactly the attribution the rule was waiting for, and it did not come from the page. ⚠️ **Sanya Pillai keeps her placeholder because the question is ANSWERED, not because it is open**: that photograph is not hers, and it must never be copied onto her entry. 🔓 **Project cells got photographs the same way, and the distinction is the useful part**: the objection was pairing a *MISA* photograph to a named client. The two that shipped are photographs **of the client** — a PepsiCo campus sign, the Casa de Luz kitchen — where the pairing is the subject rather than a claim laid over it. CapMetro has no photograph and still renders its `<Hatch>`.
- ✂️ **Per-officer LinkedIn links are HIDDEN, temporarily** (officer, 2026-08-23). One switch: `SHOW_OFFICER_LINKEDIN` in `lib/officers.ts`, read by `OfficerCard` and nothing else. 📌 **The seven URLs stay in the data** — hiding is a render decision, and deleting them would mean re-sourcing profiles from a page that no longer carries per-officer links at all. 🪤 Layout is safe either way: the card has always rendered the link conditionally, because six of thirteen officers never had one, and its `mt-auto` spacer exists so a link-less card keeps its shape. 📌 **MISA's own company page in the footer is untouched** — this is about individual people.
- 🪤 **Officer headshots are cropped SQUARE BY THE PIPELINE, not by the card.** `OfficerCard` renders `ratio="aspect-square"`, so a non-square file was already being cropped — by CSS `object-cover`, from the geometric centre, which on a portrait headshot lands on the chest and takes the top of the head off. `scripts/build-photos.mjs` crops them with `position: "north"` so the file IS the frame. 🔴 **`north` was chosen by comparing it against sharp's `attention` strategy on all twelve at once, not from first principles** — attention repeatedly picked a bright background over the face and sliced the tops off two heads. **Neither is a face detector**, so a subject who is off-centre in their source photograph stays off-centre; that is a source problem, not a setting to tune. ⚠️ **`withoutEnlargement` silently defeats `fit: "cover"`** — ask for 800×800 cover from a 385×532 source with enlargement forbidden and sharp skips the resize entirely, so the output is neither square nor cropped and looks fine in `ls`. The square edge is computed per image instead: `min(width, height, max)`. 🔓 **`CROPS` in the same file is the per-officer escape hatch** — an explicit rect in source pixels that bypasses the strategy, because a crop strategy cannot fix a FRAMING problem: a full-length shot leaves the head at ~22% of the cell where the rest of the grid is ~50%, and that reads as a mistake. 🔴 **A rect is tied to the exact file it was measured against; replace the file and delete the entry**, or it silently crops the wrong place.
- 🔓 **The officer-headshot rule was SATISFIED on 2026-08-23, and the shape of it still binds.** It was a second, independent reason: even with photography restored, the handoff's own README flagged its headshots' photo-to-name pairing as never supplied, and a real face against another real student's name is worse than an empty labelled square. The officer then supplied the pairing, so all 13 cards carry a photograph. ⚠️ **What the rule leaves behind is the test, not the outcome: a headshot ships only when something ATTRIBUTES it.** `Officer.photo` STAYS optional for exactly that reason, even now that nobody is relying on it: the shared file that blocked two cards was attributed to Daniel Chen out of band, and Sanya Pillai was then supplied a genuine photograph of her own rather than his. ⚠️ **The optionality is the rule, not a leftover of it** — the next officer added to the roster arrives without a photograph and must render `<Hatch>` rather than borrow somebody else's.
...
- **The Invariants above outrank all four, without exception.** In particular: a slot renders a photograph or a labelled `<Hatch>` and **never a hole**, and no skill's proposal to "add a hero image" authorises committing one — the image files are an officer decision; global CSS stays inside a Tailwind cascade layer; and the nav cannot grow without re-measuring the wordmark clearance.
