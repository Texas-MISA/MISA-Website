...
## 🔖 Picking this up cold (as of 2026-08-05)

Read this before touching anything; it is the state no file can tell you on its own.

| | |
|---|---|
| **Branch** | **Phase 4 is merged, pushed and deployed; there is nothing outstanding.** `main` and `stage-6-member-directory` point at the **same commit** and both are level with `origin` — the branch was fast-forwarded right after the merge, because falling behind is exactly what happened after phase 3 and it is easy to miss. `3931d99` is the `--no-ff` merge; a docs commit follows it. Phase 4 went in as two commits — `df61707` for the code and migration, `a626e22` for the docs — split that way because the 618-line doc diff had accumulated v1.30 → v1.34 across sessions and is mostly planning for work that is not built; keeping it out of the code commit leaves the phase reviewable on its own. Stage 6 merges to `main` **at the end of each phase** rather than at the end of the stage — keep doing that. |
| **Production** | **Everything through Stage 6 phase 4 is live** on https://misa-website-beta.vercel.app (deployed 2026-08-05, migration 18 pushed *before* the merge so the schema never trailed the code). Verified by response, not assumed: `/`, `/attend`, `/officers`, `/about` all 200; `/leaderboard` is **404 and that is correct** — it is Stage 7, unbuilt. anon is **401** on `member_directory` and 200 on `leaderboard` — re-check after *every* deploy, because a view recreate is the one thing that silently re-opens it, and migration 18 recreates it.<br>🪤 **Two `200`s that are not holes.** `members` and `member_field_definitions` answer anon `200` with an empty body — RLS with no policies filters rows rather than refusing the request, so **read the body, not the status**. Only the view answers 401, because a view has no RLS and the 401 comes from having no grant. `events` returns real rows by design (`events_public_read`); confirmed published-only, 15 of them.<br>⚠️ A 307 on an `/admin/*` path proves nothing about whether a route shipped — the proxy redirects paths that do not exist, and `vercel inspect` truncates its function list past the first five (re-confirmed 2026-08-05: grepping its output for `members` returns nothing at all). So **neither `/admin/members/[id]` nor phase 4's `/admin/members/fields` has been confirmed live by response.** The evidence is that the deployed commit contains them and the build succeeded. Click through once while signed in to close it properly. |
| ✅ **Database — local and remote are LEVEL again** | **19 migration files, through `…000018`, on both.** Migration 18 was pushed 2026-08-05 and confirmed by `npx supabase migration list --linked` showing `local` and `remote` equal, then by checking the objects themselves exist on the remote (the `member_field_definitions` table, `members.custom_fields` + `updated_at`, and all three appended `member_directory` columns). They had diverged for three days — the only time in this project — which was deliberate rather than drift. Earlier: phase 2 added **15** (anon revoke), **16** (the EID rename) and **17** (a data backfill); **phase 3 added none**. Next migration number is **19**, and Stage 6.5 phase 1 claims it. |
...
| **Tests** | **438 across 18 files**, lint, build and `npx tsc --noEmit` all clean. Phase 5b added `tests/xlsx.test.ts` (26 pure), which unzips the workbook in-test rather than pulling a reader in — the zero-dependency property covers the tests too, and it is asserted in the file. `tests/` is outside both the lint and the build graph, so **run tsc explicitly**; the reason it once passed vacuously is recorded under phase 3. Phase 4 added `tests/member-actions.test.ts` (integration) and extended `members`, `filters` and `validation`; phase 5a added `tests/export.test.ts` (38 pure) and an export block in `member-directory.test.ts` (integration). ⚠️ That new block owns its fixtures under the marker **`t3qex`**, a strict narrowing of `t3q`, for the reason recorded above `CF_MARKER` — earlier describes in that file create `t3q` members as they run, so a count asserted against the shared marker counts theirs too. Getting this wrong reads as an off-by-36. |
