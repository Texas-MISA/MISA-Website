...
**Stages 0–5 are complete. Stage 6 (member directory) is in progress — phases 1, 2a, 2, 3, and 4 of 9 built, merged, and deployed; phase 5 is next.** The stage was re-planned on 2026-08-01 after four design decisions landed on top of phase 1. Carry-over chores from Stage 0 are collected under Loose ends.
...
| **Next task** | **Stage 6 phase 5 — selection and extraction** (row checkboxes, "select all N matching this filter", the field picker, CSV + `.xlsx`, and the codebase's first Route Handler). Scope is below and grew in doc v1.30. ⚠️ Phase 5 is **no longer where the stage's exit criteria are met** — that moved to Stage 6.5, which interrupts between phases 5 and 6. One loose end from phase 4 first, if you want it closed: sign in to production and click through `/admin/members/fields` and `/admin/members/[id]`, neither of which has been confirmed live by response (see **Production** above). |
