...
- 🏗️ **IN PROGRESS — 📛 the PORTAL REBUILD (officer, 2026-09-18): a UI redesign of the member portal and EVERY page in it** — the `/portal` hub, `/portal/attend`, `/portal/leaderboard` and `/portal/lookup`, and any page added under `/portal` later. This is **v2 phase 3, un-deferred and widened to take in the hub**. Plan in [`docs/frontend-redesign-v2-plan.md`](docs/frontend-redesign-v2-plan.md). It still owns the `--misa-muted`-on-Vellum AA fix below, and still changes presentation only — check-in, lookup and leaderboard behaviour stay as they are. Phase 1 merges first. 🧰 **It runs through the design toolkit** (the section above): `/design-brief` for each of the four surfaces, then `/design-gate`. 🧭 **STATE 2026-09-19, on branch `design-toolkit`: parts 0, 1 and 2 are DONE and `portal-hub` is `rebuilt`** — the first of the four surfaces through brief → concepts → evidence → build → gate → rebuilt. `debeca2` re-measured all four surfaces and corrected the counts; `fa7c2e5` added the four shared primitives (`PortalBand`, `buttonClass({ touch })`, `Table scroll={false}` + `THead sticky="page"`, `StatusRegion`); `d827ac7` rebuilt the hub as concept A; `ab3f6c9` adopted the gate's findings, `e67acb8` wrote its seven receipts, `30bcec5` flipped the registry and DESIGN.md together. Check-in's row bottom **423 → 325** at 360×640 against a ≤424 bar, verified independently by the `design-reviewer` agent and by critique Assessment B. ⏭️ **Next: part 3, `/portal/attend`.**
