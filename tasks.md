...
**Stages 0–5 are complete. Stage 6 (member directory) is in progress — phases 1, 2a, 2, 3, 4 and 5 of 9 built and browser-verified; Stage 6.5 (dues) interrupts here, before phase 6.** The stage was re-planned on 2026-08-01 after four design decisions landed on top of phase 1. Carry-over chores from Stage 0 are collected under Loose ends.
...
| | |
|---|---|
| **Branch** | **Everything through Stage 6.5 phase 1 is merged, pushed and deployed; there is nothing outstanding.** `main` and `stage-6-member-directory` point at the **same commit** and both are level with `origin` — the branch was fast-forwarded right after the merge, because falling behind is exactly what happened after phase 3 and it is easy to miss. `3931d99` is the `--no-ff` merge; a docs commit follows it. Phase 4 went in as two commits — `df61707` for the code and migration, `a626e22` for the docs — split that way because the 618-line doc diff had accumulated v1.30 → v1.34 across sessions and is mostly planning for work that is not built; keeping it out of the code commit leaves the phase reviewable on its own. Stage 6 merges to `main` **at the end of each phase** rather than at the end of the stage — keep doing that. |
| **Production** | **Everything through Stage 6.5 phase 1 is live** on https://misa-website-beta.vercel.app (deployed 2026-08-06). **Migrations 18 and 19 were each pushed *before* their merge**, so the schema has never trailed the code — keep doing that. Verified by response, not assumed: `/`, `/attend`, `/officers`, `/about` all 200; `/leaderboard` is **404 and that is correct** — it is Stage 7, unbuilt; `/admin/members/export` is **307** to login when unauthenticated. anon is **401** on `member_directory` and 200 on `leaderboard` — re-check after *every* deploy, because a view recreate silently re-opens it and **both** migrations 18 and 19 recreate it.<br>🪤 **Empty `200`s that are not holes.** `members`, `member_field_definitions` and now `dues_payments` answer anon `200` with an **empty body** — RLS with no policies filters rows rather than refusing the request, so **read the body, not the status**. Only the *view* answers 401, because a view has no RLS and the 401 comes from having no grant. `events` returns real rows by design (`events_public_read`); confirmed published-only, 15 of them.<br>⚠️ A 307 on an `/admin/*` path proves nothing about whether a route shipped — the proxy redirects paths that do not exist, and `vercel inspect` truncates its function list past the first five. So **`/admin/members/[id]` and `/admin/members/fields` have still never been confirmed live by response.** The evidence is that the deployed commit contains them and the build succeeded. Click through once while signed in to close it properly — this is the oldest open loose end here. |
...
*Attendees split into **official** and **unofficial** members on whether they have paid dues (§7 Stage 6.5, doc v1.34). Planned 2026-08-05; **phase 1 of 4 built 2026-08-06** — the schema and the pure core. Phase 2 (the import) is next. It **interrupts Stage 6 between phases 5 and 6**: phase 5 ships the export machinery, this makes dues real, and Stage 6's exit criterion is then demonstrated against the real column. Numbered 6.5 rather than renumbering Stages 7–10, which would touch every stage reference in three files for no gain.*
