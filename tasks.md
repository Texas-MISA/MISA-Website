...
- **Pipeline per surface:** `/design-brief` (impeccable `shape` → the brief impeccable itself loads; `frontend-design`'s two concepts; `ui-ux-pro-max` evidence) → build → `/design-gate` review → officer. Surfaces are registered in `docs/design/surfaces.json`. ✅ **`portal-hub` is `rebuilt` (2026-09-19) — the first surface through the pipeline end to end, seven receipts passing.** The other three are `in-progress`: each has its brief, so **their files are unguarded and their builds may start**. 🔒 **A `rebuilt` surface's files are FROZEN** — `receipts.mjs` fails it as **stale** on any later commit touching them that no receipt names as a `fix_commit`, so parts 3–5 must not edit `app/(public)/portal/page.tsx`. Shared primitives in `components/ui/` are exempt by DESIGN.md and may still change.
...
- ✅ **`npm run test:ui` is RED on ONE check now, and it is part 4's.** `/portal/lookup`'s member result fails axe `definition-list` (a `<p>` inside a `<dl>` group, `lookup-form.tsx:435`). **41 pass, 1 fail** as of 2026-09-20. 🔓 **Both of `/portal/attend`'s `color-contrast` failures went green at part 3** — the `--misa-muted`-on-Vellum failure on the review step's three `<dt>` labels, fixed by moving the panel's GROUND to white and the ink to Secondary Graphite rather than recolouring a token in place. Earlier state, for the record: 39 pass / 3 fail, re-confirmed at the Portal Rebuild's part 0 (`npm test` 41 files / 1142 tests / 0 failures). Axe named them `definition-list (1): dl` and `color-contrast (3): … > dt` — **one** malformed list node and the review step's **three** labels, so neither fault was ever as wide as the prose around it implied.
...
- ✅ **`/design-brief` is DONE for all four portal surfaces (2026-09-19)**, the officer answering impeccable's `shape` interview each time. Briefs: `.impeccable/surfaces/route-portal*.md`; receipts: `docs/design/surfaces/<surface>/receipts/`. The officer adopted concept **A** on every surface and approved each surface's new copy (amending one dues sentence). **The state a build starts from — adopted shapes, gate bars, approved copy, the how-to — is [`docs/frontend-redesign-v2-plan.md`](docs/frontend-redesign-v2-plan.md) §Phase 3 brief.** Merge order: `portal-phase-1`, then this branch.
  - 🔴 **THE PORTAL HAS NO NAVY HEADER (officer, 2026-09-20).** Every `/portal` page is one white `.sheet` on the grey ground, its title a masthead above a bleeding rule (`components/ui/portal-sheet.tsx`). `PortalBand` is deleted. **Every "short band" below is superseded** — the briefs carry an OFFICER OVERRIDE block saying the same thing.
  - ✅ **Build `/portal`** — DONE, `rebuilt`, and **RE-GATED against the sheet (round 2, `92076ac` → `91e5c13`)**. One shared-rule plate of three whole-row links, now bleeding inside the sheet. Bar met: check-in row bottom **267.2px** at 360×640 against ≤424 (423 before the rebuild), two taps from the header. 🔓 The recorded **265.5 does not reproduce** — 267.2, confirmed three ways. 🔴 The re-gate found the focus ring this surface's own gate had "fixed" **was never painting**.
  - ✅ **Build `/portal/attend`** — DONE, `rebuilt`, and **RE-GATED against the sheet (round 3, `b2900e1` → `5d8731d`)**. Tighter form, the new box label plus its reassurance line, all twelve states designed and gated. Bar met under **both** scrollbar conventions: **591.5px at a true 360×640** (48.5px slack) and **611.5 / 607.66 in a 360px desktop window** (cut at 668 before). Both muted-on-Vellum occurrences gone. 🪤 **Every record of this bar must name its layout width** — there is a cliff at 348px where the checkbox label wraps and the row grows 20px.
  - ⬜ **Build `/portal/leaderboard`** — one table, top 10 larger above a navy cut line, the column head sticky below the site header. ✅ Its concept already wanted no navy hero, so the officer override costs it nothing. Bar: the whole top 10 on the first screen at 1280×720; names never below 16px.
  - ⬜ **Build `/portal/lookup`** — one sheet ordered points → dues → events, stacked event rows below `sm`. Bar: field and button on the first phone screen; no sideways scroll to read whether you attended; the `definition-list` axe fault gone. 🔴 It carries **nine of the eleven** remaining muted occurrences and the **one** red `test:ui` check.
  - ⬜ **Then `/design-gate <surface>` for each** — ✅ done for `portal-hub` and `portal-attend` (seven receipts each, plus an `officer.md` for the header removal). Flip `status` to `rebuilt` in `docs/design/surfaces.json` and update DESIGN.md's surface table in the same commit, and keep `npm run test:ui` green on the surface.
...
`/attend`, `/leaderboard` and `/lookup` moved to `/portal/attend`, `/portal/leaderboard` and
`/portal/lookup`, and a static hub now sits at `/portal`. The old paths are **permanent 308
...
- 🔶 **Preview:** pushed; the branch's stable preview is https://misa-website-git-portal-phase-1-txmisa-jds-projects.vercel.app. It sits behind the Vercel login, so curl and the automated browser both get the SSO page and only a signed-in officer can check it. 🔴 **No check-ins:** the preview writes to production's database.
  - ⚠️ **Open: the officer could not see the MISA logo (the link home) in the preview** (2026-09-18). It could not be reproduced. The branch renders it at true 320, 360 and 390px phone viewports and on desktop: the mask image returns 200, and the logo is the topmost element at its centre. Production renders it with the same `.wordmark` CSS. Waiting on a screenshot, or on the officer signing in to Vercel in the automated browser.
- ⬜ **Merge:** only on the officer's go-ahead, and not on an event day. First confirm no check-in window is open (`npx supabase db query --linked`, one line). Use `git merge --no-ff`, then smoke-test production (including `https://txmisa.org/attend` without `www`).
- 🪤 **Rollback:** Vercel Instant Rollback to `ce5bda7`, then `git revert --no-edit f5d2d85..portal-phase-1` on `main`. **Never `git revert -m 1 <merge>`**: the branch also carries the v1.80 docs and the plan, which describe live behaviour. After an Instant Rollback, promote the revert's deployment by hand.
- 🔴 **The local seed had been broken since early September, and `f5d2d85` fixes it.** Its "still to come" Fall Kickoff (1 September) had become past, so the seed's own assert failed. That rolled back the whole seed, so `db reset` produced an EMPTY local database and 23 tests failed against it. Both upcoming events now sit in December, and **they expire again on 1 December 2026**.
- 🏗️ **IN PROGRESS — 📛 the PORTAL REBUILD (officer, 2026-09-18): a UI redesign of the member portal and EVERY page in it** — the `/portal` hub, `/portal/attend`, `/portal/leaderboard` and `/portal/lookup`, and any page added under `/portal` later. This is **v2 phase 3, un-deferred and widened to take in the hub**. Plan in [`docs/frontend-redesign-v2-plan.md`](docs/frontend-redesign-v2-plan.md). It still owns the `--misa-muted`-on-Vellum AA fix below, and still changes presentation only — check-in, lookup and leaderboard behaviour stay as they are. Phase 1 merges first. 🧰 **It runs through the design toolkit** (the section above): `/design-brief` for each of the four surfaces, then `/design-gate`. 🧭 **STATE 2026-09-20, on branch `design-toolkit`: parts 0–3 are DONE and TWO of the four surfaces are `rebuilt`** — `portal-hub` and `portal-attend`, each through brief → concepts → evidence → build → gate → rebuilt with seven passing receipts. `debeca2` re-measured all four surfaces and corrected the counts; `fa7c2e5` added the four shared primitives (`PortalBand` — since deleted, see below, `buttonClass({ touch })`, `Table scroll={false}` + `THead sticky="page"`, `StatusRegion`); `d827ac7` → `30bcec5` built and gated the hub (check-in row bottom **423 → 325** at 360×640 against a ≤424 bar); `a38a2b3` → `4c216f1` built and gated `/portal/attend` (Check in button bottom **668 → 610.5 → 607.7** against a 640 fold, measured independently twice, all twelve states designed and gated, both muted-on-Vellum occurrences gone, `test:ui` 39/3 → **41/1**). Part 3 also answered the phase's one open copy question — the unmatched banner's replacement wording, approved by the officer 2026-09-19. 🔴 **THE PORTAL HAS NO NAVY HEADER as of 2026-09-20** (officer, against a screenshot of `/portal` on the preview): the band is removed from ALL FOUR portal pages and replaced by *the sheet is the page* — each page is one white `.sheet` on the grey ground, its title a masthead above a bleeding rule (`ca5cd06`). `PortalBand` is DELETED; `components/ui/portal-sheet.tsx` replaces it with four callers. Both frozen surfaces carry a `receipts/officer.md` naming that commit, or the checker fails them as stale. 🔴 **The first build LOST check-in’s bar** (647.7 against a 640 fold, worse than the 610.5 the band version had); removing a back link that duplicated the site header’s MEMBER PORTAL button (−40px) and tightening the sheet’s phone padding brought it to **607.7**, and the hub went **325 → 265.5**. 🔓 **BOTH of those figures were re-derived at the re-gates and both moved** — the hub is **267.2**, and check-in's bar is **two numbers**: 591.5 at a true phone 360×640, 611.5/607.66 in a 360px desktop window (classic scrollbar → 345–346 layout width → the checkbox label wraps). Both clear the fold; every record must name its convention. ✅ **ROUNDS 1–3 ARE DONE (2026-09-20), HEAD `5d8731d`, 10 commits ahead of `main`, unpushed:** round 1a landed the three shared-primitive fixes (`f2e41d6` the WCAG 1.4.11 input boundary via a new `--misa-control-edge`, `b01dc6a` the header button to a 48px phone floor, `539a5f0` `activities.tsx` to `size="card"`); round 1b applied the officer's "the interface does not explain itself" rule to the frozen pair and **correctly found nothing to delete**, with a 42-sentence inventory as the evidence; `77d2c56` fixed a `.gitattributes` defect that had silently stopped `tests/design-receipts.test.ts` from loading at all; and rounds 2 and 3 **RE-GATED both frozen surfaces against the sheet build**, because their original seven receipts had reviewed `a38a2b3` — a build with a navy band that no longer exists. 🔴 **An officer receipt cures staleness; it does not constitute a review.** ⏭️ **Next: part 4, `/portal/lookup`**, which carries nine of the eleven remaining muted occurrences and the one red `test:ui` check. **The handoff prompt for parts 4–7 is [`docs/handoff-portal-rebuild-rounds-4-7.md`](docs/handoff-portal-rebuild-rounds-4-7.md).**

🔴 **Two system rules came out of the hub's gate and parts 3–5 inherit both** (recorded in `DESIGN.md` §Components):
  - **A focus ring must contrast with every ground the FOCUSED ELEMENT spans**, not just the one its section sits on. `.on-navy :focus-visible` answers "a focused element *inside* a navy section"; the hub's row link was the first thing here to span two grounds inside ONE focusable element, and its navy ring measured **1.00:1** over the navy key. Check-in and lookup both take the band and the row idiom, so the next navy-element-inside-a-link hits it again.
  - **`<Title className="text-[22px] …">` silently does nothing.** Arbitrary-value utilities tie on specificity and Tailwind v4 sorts them **ascending**, so the larger always wins — use the new `size="card"` prop. 🐛 The same bug is **live on the home page** via `components/ui/activities.tsx:98`, which renders **34px** where the code asks for 26, and has since the bento grid was built. **Part 6 owns that fix**, not a portal surface.

🔴 **The parts skeleton was REVISED** — each surface's contrast fix now lands inside its own build part, because a late sweep makes every receipt **stale**; read *The build, in parts* before picking the work up. 🔴 **The parts skeleton was REVISED** — each surface's contrast fix now lands inside its own build part, because a late sweep makes every receipt **stale**; read *The build, in parts* before picking the work up.
...
Plan in [`docs/frontend-redesign-v2-plan.md`](docs/frontend-redesign-v2-plan.md). **✅ Phase 0 complete. ✅ Phase 1 (home page + header) COMPLETE — gate passed 2026-08-19 after four rounds of officer review. ✅ Phase 2 (the five content pages) — LIVE. ✅ Phase 4 (`/admin`) — built and measured 2026-08-29, its five held-back findings decided and built 2026-08-31, and **MERGED AND LIVE 2026-08-31**. ⏭️ Phase 3 is NEXT (officer, 2026-09-18): the UI redesign of the member portal and every page in it — `/portal`, `/portal/attend`, `/portal/leaderboard`, `/portal/lookup` — run through `DESIGN.md` §Design toolkit. It is the only phase left before 5.**

✅ **PHASE 4 IS MERGED AND LIVE** (officer, 2026-08-31) — 18 commits
fast-forwarded from `v2-phase-4-admin`, which was cut from the roster-terms
commit rather than from `main`, so the screens it styles are the term-aware ones
migration 29 produced. **That cut is why the merge needed a schema push first:**
migration 29 was still absent from production, and the code depends on it.

### ✅ Phase 4 — `/admin`. MERGED AND LIVE. (built 2026-08-29, shipped 2026-08-31)
...
✅ **PHASE 2 IS LIVE.** It reached `main` before 2026-08-31 and is an ancestor of the current head; `git rev-list --count v2-phase-2 ^main` is **0**. *(This line said phase 2 lived only on the branch and must not be merged, which stopped being true without the line moving.)*
...
⏭️ **Phase 3 is now SCHEDULED as the next task (officer, 2026-09-18)**, as the UI
redesign of the member portal and every page in it. The pages below live under
`/portal` now, and this table is its checklist.
...
none: its one line of small text on the grey ground uses `--misa-secondary`.
