...
**Stages 0–5 are complete. Stage 6 (member directory) — phases 1 through 8 of 9 built and browser-verified, and ✅ its exit criteria are MET. Stage 6.5 (dues) interrupted between 5b and 5c and is ✅ COMPLETE — all 4 phases. Only phase 9, the closing read-through, remains.** The stage was re-planned on 2026-08-01 after four design decisions landed on top of phase 1. Carry-over chores from Stage 0 are collected under Loose ends.
...
| **Next task** | **Stage 6 phase 9 — docs and a closing read-through**, and then the stage is done. Stage 5's read-through found four pieces of drift in one pass, all in docs nobody had reread — and phase 8 just found a fifth of exactly that kind, so the pass earns its keep: the architecture doc and `tasks.md` had both asserted for months that a merge would trip `attendance_one_per_event`, and it structurally cannot. **Read the invariants against the code, not from memory.**<br>✅ **Phases 1 through 8 of 9 are done**, Stage 6's exit criteria are met, and Stage 6.5 is complete.<br>📌 One loose end stays open and cannot be closed from here: `/admin/dues/[id]` has never been confirmed live by response, because production holds no payments and an invented id returns the same generic 404 a non-existent route does. It closes for free the first time a real statement is imported (see **Production**). |
...
| **Tests** | **736 across 26 files**, lint, build and `npx tsc --noEmit` all clean.<br>Phase 8 added 41: `tests/merge.test.ts` (34 pure) and `tests/member-merge-actions.test.ts` (7 integration on the `t3qmrg` marker).<br>🔓 **The headline integration test proves the HAZARD before proving the fix** — a naive repoint of two members who both attended one event returns **no error** and yields `events_attended = 2` for one distinct event with doubled points; the reject-first sequence yields 1. Two more pin the FK behaviours the merge is ordered around: deleting a member **cascades their adjustments away**, and **set-nulls their attendance**, while only a dues payment blocks with 23503.<br>⚠️ **Two of the pure tests are SOURCE assertions**, guarding walkthrough defects no behavioural test in a node environment can reach: `joined_at` in `AUDITED_MEMBER_COLUMNS`, and the merge picker living outside the form React resets.<br>📌 The ranker's floor was **measured, not reasoned** — and the seed could not supply it (zero member-vs-member pairs above 15 across all 496), so the shapes were constructed. The measured table is on `MIN_DUPLICATE_SCORE` and re-asserted as `it.each` cases either side of the line.<br>Earlier: 695 across 24 files after phase 7; 619 across 21 before it. |
