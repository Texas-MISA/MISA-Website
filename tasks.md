...
| | |
|---|---|
| **Branch** | ✅ **Everything is merged, pushed and deployed; nothing is outstanding and the working tree is clean.** `main` and `stage-6-member-directory` both point at **`2c38084`** (Stage 6 phase 9, the stage close) and both are level with `origin` — all four refs checked, not assumed. Phase 9 needed **no migration**, confirmed with `migration list --linked` showing 21 files and zero mismatched.<br>📌 **Stage 6 is closed.** The branch has served eleven phase-commits; Stage 7 should start from a fresh branch off `main`.
| **Production** | ✅ **Everything through Stage 6 phase 9 is live** on https://misa-website-beta.vercel.app (deployed 2026-08-08 as `2c38084`). Schema level at `…000020`.<br>📌 **Phase 9 changed no application behaviour** — it is docs plus `seed.sql`, and `seed.sql` does not ship — so there was no new route to verify by rendered response. What was checked is that nothing regressed: `/`, `/attend`, `/officers`, `/about` all **200**; `/leaderboard` **404**, correct until Stage 7.<br>Anon re-checked: **401** on `member_directory`, **200** on `leaderboard` carrying only `id` / `full_name` / `total_points`, empty **200**s on `members`, `member_filter_presets` and `dues_payments`.<br>⚠️ **Production was NOT re-seeded**, though phase 9 fixed the wipe list that made it necessary. It still carries the stray `shirt_size` definition — see the carried-out items. A re-seed is destructive and the officer authorises it separately.<br>Earlier: everything through phase 8 was live as `9aac732`.
