...
- ✅ **`/design-brief` is DONE for all four portal surfaces (2026-09-19)**, the officer answering impeccable's `shape` interview each time. Briefs: `.impeccable/surfaces/route-portal*.md`; receipts: `docs/design/surfaces/<surface>/receipts/`. The officer adopted concept **A** on every surface and approved each surface's new copy (amending one dues sentence). **The state a build starts from — adopted shapes, gate bars, approved copy, the how-to — is [`docs/frontend-redesign-v2-plan.md`](docs/frontend-redesign-v2-plan.md) §Phase 3 brief.** Merge order: `portal-phase-1`, then this branch.
  - ⬜ **Build `/portal`** — one shared-rule plate of three whole-row links under a short field band. Bar: the check-in row's bottom ≤ 424px at 360×640, two taps from the header.
  - ⬜ **Build `/portal/attend`** — short band, tighter form, new box label plus its reassurance line. Bar: the Check in button fully on the first screen at 360×640 (cut at 671 today); 8px is the floor on any gap.
  - ⬜ **Build `/portal/leaderboard`** — no navy hero, one table, top 10 larger above a navy cut line, the column head sticky below the site header. Bar: the whole top 10 on the first screen at 1280×720 (6 rows today); no name under 16px (14px today).
  - ⬜ **Build `/portal/lookup`** — short band, one sheet ordered points → dues → events, stacked event rows below `sm`. Bar: field and button on the first phone screen; no sideways scroll to read whether you attended; the `definition-list` axe fault gone.
  - ⬜ **Then `/design-gate <surface>` for each**, flip its `status` to `rebuilt` in `docs/design/surfaces.json` and update `DESIGN.md`'s surface table in the same commit, and keep `npm run test:ui` green. 🪤 Merge the branch, never squash: the receipts name commits that must survive.
