...
- [x] **Add partner logos** — done. `public/partners/` carries the four real marks (KPMG, PwC, EY, PepsiCo), each from the company's own published brand kit and used per those guidelines. 🔓 The partner list was confirmed and changed by the officer on 2026-09-20: ConocoPhillips and Credera are out, EY and PepsiCo are in. `scripts/build-partner-logos.mjs` normalises all four onto one 1240x420 canvas so the row reads as one size — see the header comment in `components/ui/partners.tsx` for why the `h-*` there sizes the canvas rather than the mark.
