...
| | |
|---|---|
| **Branch** | ⚠️ **Stage 6.5 phase 3 is built on `stage-6-member-directory` and NOT yet merged, pushed or deployed.** Everything through phase 2 is merged, pushed and deployed. Phase 3 needs no migration, so the "push the schema before the merge" step does not apply this time — the merge and deploy are the only outstanding work.<br>Previously: `main` and `stage-6-member-directory` pointed at the **same commit**, both level with `origin`. Fast-forward the branch right after every merge — falling behind is exactly what happened after Stage 6 phase 3 and it is easy to miss. Merge to `main` **at the end of each phase** rather than at the end of the stage. |
| **Production** | **Everything through Stage 6.5 phase 2 is live** on https://misa-website-beta.vercel.app (deployed 2026-08-06). **Migrations 18 and 19 were each pushed *before* their merge**, so the schema has never trailed the code — keep doing that. Verified by response, not assumed: `/`, `/attend`, `/officers`, `/about` all 200; `/leaderboard` is **404 and that is correct** — it is Stage 7, unbuilt; `/admin/members/export` and `/admin/dues/import` are both **307** to login when unauthenticated. anon is **401** on `member_directory` and 200 on `leaderboard` — re-check after *every* deploy, because a view recreate silently re-opens it and **both** migrations 18 and 19 recreate it.<br>🪤 **Empty `200`s that are not holes.** `members`, `member_field_definitions` and now `dues_payments` answer anon `200` with an **empty body** — RLS with no policies filters rows rather than refusing the request, so **read the body, not the status**. Only the *view* answers 401, because a view has no RLS and the 401 comes from having no grant. `events` returns real rows by design (`events_public_read`); confirmed published-only, 15 of them.<br>⚠️ A 307 on an `/admin/*` path proves nothing about whether a route shipped — the proxy redirects paths that do not exist, and `vercel inspect` truncates its function list past the first five. So **`/admin/members/[id]` and `/admin/members/fields` have still never been confirmed live by response.** The evidence is that the deployed commit contains them and the build succeeded. Click through once while signed in to close it properly — this is the oldest open loose end here. |
