...
| | |
|---|---|
| **Branch** | 🔨 **`stage-8-hardening`, cut from `main`.** Stage 7 is closed, both phases.<br>**Merge to `main` at the end of each phase.**<br>📌 A docs-only commit still triggers a Vercel build, so the deployed commit and this row can differ by one whenever the last change was paperwork.
...
### Phase 2 — attendance and adjustment export for archival (next)

- Reuse `toCsv` / `toTsv` / `toXlsx` / `guardFormula` / `ExportCell` untouched — all three writers consume exactly `(ExportField[], ExportCell[][])`. Write new: a catalogue and a `projectRow` per entity.
- ⚠️ `parseFieldSelection` hardcodes `DEFAULT_EXPORT_FIELDS` (`lib/export.ts:250`) — needs a defaults parameter.
- ⚠️ **The audit vocabulary moves in one commit.** `AuditEntityType` and the SQL CHECK have drifted **twice**. An N-row export cannot file under `attendance` or `point_adjustment`; it needs a new entity type on the `roster` precedent, so **migration 23**.

### Phase 3 — error boundaries, loading and empty states
