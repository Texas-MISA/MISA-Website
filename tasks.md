...
| **Next task** | **Stage 6.5 phase 1 — dues & membership status**, which interrupts Stage 6 between phases 5 and 6 (doc v1.34, spec [`docs/dues-and-membership.md`](docs/dues-and-membership.md)). Phase 5 is complete: 5a shipped selection/catalogue/clipboard/CSV, 5b the xlsx workbook. Migration **19** is the next number and Stage 6.5 phase 1 claims it; both pre-build blockers were cleared 2026-08-05. ⚠️ Phase 5 is **no longer where the stage's exit criteria are met** — that moved to Stage 6.5 phase 4's calculated dues column. Loose end still open from phase 4: sign in to production and click through `/admin/members/fields` and `/admin/members/[id]`, neither confirmed live by response (see **Production**). |
