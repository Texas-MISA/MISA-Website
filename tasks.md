...
| | |
|---|---|
| **Branch** | 🔨 **`stage-7-member-facing-views`, cut from `main` at `ea54184`.** Stage 6 is CLOSED and `stage-6-member-directory` has nothing left to do.<br>**Merge to `main` at the end of each phase**, the way Stage 6 did rather than at the end of the stage.<br>📌 A docs-only commit still triggers a Vercel build, so the deployed commit and this row can differ by one whenever the last change was paperwork.
| **Production** | ✅ **Everything through Stage 7 phase 1 is live** on https://misa-website-beta.vercel.app (deployed 2026-08-09 as `24797d5`). Schema level at **`…000021`**.<br>**`/leaderboard` verified live by rendered response**: 200, `<meta name="robots" content="noindex, nofollow">` present, heading reads Fall 2026, 29 distinct members, ranks 1/2/3. `/`, `/attend`, `/officers`, `/about`, `/contact`, `/gallery`, `/projects` all **200**; `/lookup` **404**, correct until phase 2. Nav link renders on `/`.<br>Anon re-checked after the push: `leaderboard` returns exactly `id` / `full_name` / `total_points` / **`term`** and no identifier, `rpc/current_term` answers, **401** on `member_directory`, empty **200** on `app_settings` — the definer function exposes the term and nothing around it.<br>Earlier: everything through Stage 6 phase 9 was live as `2c38084`.<br>📌 **Phase 9 changed no application behaviour** — it is docs plus `seed.sql`, and `seed.sql` does not ship — so there was no new route to verify by rendered response. What was checked is that nothing regressed: `/`, `/attend`, `/officers`, `/about` all **200**; `/leaderboard` **404**, correct until Stage 7.<br>Anon re-checked: **401** on `member_directory`, **200** on `leaderboard` carrying only `id` / `full_name` / `total_points`, empty **200**s on `members`, `member_filter_presets` and `dues_payments`.<br>⚠️ **Production was NOT re-seeded**, though phase 9 fixed the wipe list that made it necessary. It still carries the stray `shirt_size` definition — see the carried-out items. A re-seed is destructive and the officer authorises it separately.<br>Earlier: everything through phase 8 was live as `9aac732`.
| ✅ **Database — local and remote LEVEL** | **22 migration files, through `…000021`, on both** — confirmed 2026-08-09 with `migration list --linked` showing all 22 rows matched, not inferred. Next number is **22**, unclaimed.<br>📌 **This row used to say "next number is 22" while the highest was `…000020`** — it was counting *files* (21, including `00000000000000_stage0_check.sql`) rather than the sequence. Phase 1 correctly took **21**. Count files for the file count; read the highest suffix for the next number.<br>**Migration 21 (`current_term_visibility`)** makes `current_term()` `security definer` and appends `term` to `leaderboard`. Two `create or replace` statements, no data touched, no drop — so grants survived and none are restated.<br>🔓 **`seed.sql`'s wipe list gained `member_field_definitions` and `member_filter_presets` in phase 9**, and both are now asserted by the seed's own check block. That is the fix for the `shirt_size` drift recorded two rows down — but **production has not been re-seeded**, so the stray definition is still there until someone authorises one. |
...
| **Next task** | **Stage 7 phase 2 — `/lookup`.** ✅ Phase 1 (`/leaderboard`) is built and browser-verified; see the Stage 7 section below for what it found.<br>📌 **Phase 1 left one deliberate loose end for phase 2 to close:** `/leaderboard` carries a comment where a link to `/lookup` belongs, and the nav has no "My Attendance" entry. Both were held back because each phase merges to `main` and deploys, so linking `/lookup` in phase 1 would have shipped a 404 to production for the length of a phase.<br>🔓 **Two privacy carry-overs, and both are decided rather than open — do not re-litigate them, and read the Stage 7 entry under **Later** for the rest.** `/leaderboard` is public and **must never be indexed** (§9 #1 — `robots: { index: false, follow: false }`, the pattern is already in `app/admin/(shell)/layout.tsx`); it is the one privacy choice in this project that cannot be undone by shipping a fix, because a search cache outlives the deploy that filled it. And **no unauthenticated route may return an email or an EID** — `leaderboard` carries `id`, `full_name` and `total_points` and nothing else, which `tests/security.test.ts` pins.<br>📌 `/lookup` is identity-based with no account (§2.3), so it re-opens the §6 membership-oracle question that `/attend` settled deliberately — read `docs/attend-confirmation-flow.md` before designing it rather than after. |
...
## 🔨 Stage 7: Member-facing views (in progress)

*Goal: members can answer their own questions. Exit: a member can determine their own standing, why it is what it is, which specific events they missed, and whether anything of theirs is still pending — without asking an officer. Two phases, each merged to `main` on completion.*
...
- 🔓 The gate is EID **and** matching email, expressed as **one query with both predicates**. `/attend`'s `findMember` is an ordered *fallback* (EID, then email); copying that shape silently reduces this to EID-alone, which is precisely the gate §6 says is **not** sufficient to reveal dues status.
- 🔓 One generic failure for every miss — never "no such EID" versus "that email doesn't match", which would hand back a stronger oracle than `/attend`'s.
- 🔓 Return no identifier the caller did not already supply, so §6's "emails and EIDs never returned to unauthenticated clients under any route" stays literally true.
- 🔓 Its **own** throttle bucket. `RATE_LIMIT_MAX` (90) is a *room capacity* — a member checking their standing at a meeting must not consume a check-in slot behind the venue's NAT.
- Read `docs/attend-confirmation-flow.md` before designing it, not after.
...
- 🔨 **Stage 7 — Member-facing views** · in progress, see the section above · phase 1 `/leaderboard` ✅, phase 2 `/lookup` next
  - 📌 `/admin/audit` was **considered and deliberately left out** (decided 2026-08-09). It is officer-facing, so it does not belong to this stage by theme, and its carried-out item keeps its own trigger. Revisit when someone actually asks "what happened to that member?" about a merge.
  - 🪤 ~~Add the `/leaderboard` path to `revalidatePoints`~~ — **resolved in phase 1 by not doing it.** Five modules move public standings, so the board is `force-dynamic` and there is no cache to revalidate. See the invariant in `CLAUDE.md`.
