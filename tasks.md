...
| **Next task** | **Stage 6.5 phase 4 — the directory column · Stage 6's exit criteria are met here.** `MemberFilter` gains `dues: "paid" \| "unpaid" \| "all"`, translated in `applyMemberFilter` and nowhere else; `dues` joins `MEMBER_SORTS`; the column renders **Paid / Not Paid**; the member detail page grows a payment history ordered by `termsFrom` semantics; and dues status becomes one more entry in phase 5's export catalogue. Then **Stage 6 phase 5c** (filter the directory by categorical fields), which is sequenced after 6.5 because it filters on the dues column. ⚠️ Phase 3 is **committed but not yet merged or deployed** — see **Branch**. Loose end still open from Stage 6 phase 4: sign in to production and click through `/admin/members/fields` and `/admin/members/[id]`, neither confirmed live by response (see **Production**). |
| 🚧 **Stage 6.5 — phases 1, 2 and 3 of 4 built** | **Dues & membership status** (planned 2026-08-05 doc v1.34; phases 1–3 built 2026-08-06, doc v1.40). Spec: [`docs/dues-and-membership.md`](docs/dues-and-membership.md). Built: migration 19, the term arithmetic, `lib/dues.ts`, the directory column and the reserved keys (phase 1); `app/actions/dues.ts`, `lib/dues-roster.ts` and `/admin/dues/import` (phase 2); the ledger at `/admin/dues`, the editor at `/admin/dues/[id]`, `savePayment` / `voidPayment`, and the Dues nav entry re-pointed at the ledger (phase 3). **Unbuilt: phase 4 (the directory column's UI, the `dues` filter and sort, the detail page's payment history, the export catalogue entry).** It interrupts Stage 6 between phases 5 and 6 and re-points Stage 6's exit criterion at the calculated dues column. 📌 Phase 3 shipped **two** corrections where the spec named three — see the phase-3 block for why the row-level CAS token forces that. |
| 📋 **Also planned: Stage 6 phase 5c** | **Filter the directory by categorical fields** — custom fields, dues status, and `source`. Requested 2026-08-06; documented, unbuilt. ⚠️ Sequenced **after** Stage 6.5 because it filters on the dues column. Full entry under Phase 5c below. |
...
### Phase 5c — filter by categorical fields *(planned 2026-08-06, unbuilt)*
...
⚠️ **Sits AFTER Stage 6.5, not before it**, and the ordering is not arbitrary: `dues_paid_current_term` has to exist before it can be filtered on, and "paying member / not" is the categorical officers will reach for most.

- [ ] **Custom fields** — one dropdown per `show_in_directory` definition, its options from the definition. The phase-4 spike already proved `.eq("custom_fields->>key", value)` works *through the view*, so this is cheap; the work is UI, not query.
- [ ] **`dues_paid_current_term`** — Paid / Not Paid / All, shaped exactly like the existing `state` selector.
- [ ] **`source`** — admin-created vs self-registered. Phase 3 removed it as a *column* and that was right; it belongs back as a **filter**, since §4.2's roster-cleanup query ("find the self-registered ghosts") has had nowhere to live since.
- [ ] **Every predicate goes in `applyMemberFilter` and nowhere else** (§4.5's one-translation rule). That is what makes "filter to M, then export" provably the same query as the count on screen — and it means the export, the clipboard and the xlsx all inherit the filter for free, with no second code path.
- [ ] 🔓 **A custom-field key reaching a filter string is the same injection surface as the `order=` term.** It goes through the `customSortColumn()` validation rather than being trusted for having come out of the database — the phase-4 spike showed a space and a `"` are accepted silently by PostgREST with no error at all.
- [ ] ⚠️ **A filter with no control on screen is the phase-1 defect from the other side.** A field that is archived, or has `show_in_directory = false`, must not remain filterable from a stale bookmark — same fallback the `cf:` sort keys already take.
- [ ] Tests: the recorder fake in `tests/filters.test.ts` gains the new predicates, and an integration case asserts the filtered count equals what the export returns.
