...
| **Next task** | **Stage 6.5 phase 2 — the import** (`/admin/dues/import`, `previewImport` / `commitImport`, the review queue). Phase 1 shipped the schema and the pure core; migration **19** is applied locally and pushed. Then **phase 5c** — filter the directory by categorical fields (custom fields, dues status, source) — which is deliberately sequenced after 6.5 because it filters on the dues column. Loose end still open from Stage 6 phase 4: sign in to production and click through `/admin/members/fields` and `/admin/members/[id]`, neither confirmed live by response (see **Production**). |
| 🚧 **Stage 6.5 — phase 1 of 4 built** | **Dues & membership status** (planned 2026-08-05 doc v1.34; phase 1 built 2026-08-06 doc v1.38). Spec: [`docs/dues-and-membership.md`](docs/dues-and-membership.md). Built: migration 19, the term arithmetic, `lib/dues.ts`, the directory column, the reserved keys. **Unbuilt: phase 2 (the import), phase 3 (the ledger and editor screens), phase 4 (the directory column's UI).** It interrupts Stage 6 between phases 5 and 6 and re-points Stage 6's exit criterion at the calculated dues column. |
