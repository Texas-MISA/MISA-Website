...
| **Next task** | **Stage 6 phase 5b — the xlsx workbook.** Phase 5a (selection, the field catalogue, clipboard, CSV, and the codebase's first Route Handler) is built and browser-verified; the writer choice is settled — **hand-rolled, zero dependencies** — with the reasoning and the rejected candidates recorded under Phase 5b below. ⚠️ Phase 5 is **no longer where the stage's exit criteria are met**; that moved to Stage 6.5, which interrupts between phases 5 and 6. Loose end still open from phase 4: sign in to production and click through `/admin/members/fields` and `/admin/members/[id]`, neither confirmed live by response (see **Production**). |
...
- [ ] `lib/xlsx.ts` — CRC32 + local file headers + central directory + EOCD over `deflateRawSync`; parts `[Content_Types].xml`, `_rels/.rels`, `xl/workbook.xml`, `xl/_rels/workbook.xml.rels`, `xl/worksheets/sheet1.xml`, `xl/styles.xml`. Inline strings (`t="inlineStr"`), so there is no shared-strings table to keep in step.
- [ ] **Numbers as numbers, dates as dates** via serials plus a date `numFmt`. An empty cell omits the `<c>` element entirely — a null `attendance_rate` must never become `0`.
- [ ] Consumes the **same `projectRow` output as CSV**; only the formatting differs. ⚠️ The two writers must not share a "join it with commas" shortcut — xlsx is not exposed to formula injection precisely because its cells are typed, and that asymmetry is the reason.
- [ ] Sheet named for the filter, sanitized to Excel's rules (≤31 chars, none of `[]:*?/\`).
- [ ] `format=xlsx` joins `EXPORT_FORMATS` in `lib/export.ts` — one line, since the route already switches on it.
- [ ] `tests/xlsx.test.ts` — unzip the produced bytes in-test (`inflateRawSync`) and assert the parts, the header row, a number cell carrying `<v>` with no `t="inlineStr"`, a null rate producing no cell at all, and the sheet name sanitized.
