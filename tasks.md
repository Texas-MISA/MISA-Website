...
- [ ] **Require 2FA org-wide** — **blocker cleared, flag still unset** as of 2026-07-30. `filter=2fa_disabled` now returns **0** (it previously returned `TXMISA-JD`), so both accounts have 2FA. But `two_factor_requirement_enabled` still reads `false`, and two PATCH attempts have not changed it. GitHub's refusal is safe — it rejects rather than removing anyone — so the likely cause is an attempt made before GitHub saw the member state as compliant.
  - **Re-run it and read the output.** A silent non-change means an error was returned, not that it applied. Verify afterwards rather than assuming:
  ...
  - If it still refuses, use the dashboard instead: **Organization settings → Authentication security → "Require two-factor authentication…"**. Newer GitHub rollouts of this setting involve a grace period rather than an immediate flip, which the legacy API field may not express.
