...
| | |
|---|---|
| **Branch** | **Everything through Stage 6.5 phase 2 is merged, pushed and deployed; nothing is outstanding and the working tree is clean.** `main` and `stage-6-member-directory` point at the **same commit** and both are level with `origin`. Fast-forward the branch right after every merge — falling behind is exactly what happened after Stage 6 phase 3 and it is easy to miss. Merge to `main` **at the end of each phase** rather than at the end of the stage. |
...
| **Next task** | **Stage 6.5 phase 3 — the ledger and the editor** (`/admin/dues`, `/admin/dues/[id]`, the needs-review queue, and the `assignPayment` / `setPaymentTerms` / `voidPayment` corrections). Phase 2's import can already queue rows that need an officer; phase 3 is where they get resolved, so it is the natural next step. ⚠️ **Re-point the "Dues" nav entry at `/admin/dues`** when the ledger exists — it points at `/admin/dues/import` for now. Then phase 4, then **Stage 6 phase 5c** (filter the directory by categorical fields), which is sequenced after 6.5 because it filters on the dues column. Loose end still open from Stage 6 phase 4: sign in to production and click through `/admin/members/fields` and `/admin/members/[id]`, neither confirmed live by response (see **Production**). |
| 🚧 **Stage 6.5 — phases 1 and 2 of 4 built** | **Dues & membership status** (planned 2026-08-05 doc v1.34; phases 1–2 built 2026-08-06, doc v1.39). Spec: [`docs/dues-and-membership.md`](docs/dues-and-membership.md). Built: migration 19, the term arithmetic, `lib/dues.ts`, the directory column and the reserved keys (phase 1); `app/actions/dues.ts`, `lib/dues-roster.ts`, `/admin/dues/import` and the Dues nav entry (phase 2). **Unbuilt: phase 3 (the ledger, the payment detail page, the review queue, `assignPayment` / `setPaymentTerms` / `voidPayment`), phase 4 (the directory column's UI).** It interrupts Stage 6 between phases 5 and 6 and re-points Stage 6's exit criterion at the calculated dues column. |
