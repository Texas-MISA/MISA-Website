...
| **Next task** | **Stage 7 — member-facing views: `/leaderboard` and `/lookup`.** ✅ **Stage 6 is closed, all 9 phases**, and Stage 6.5 with it.<br>🔓 **Read the two carry-overs below before starting.** `/leaderboard` is public and **must never be indexed** (§9 #1 — `robots: { index: false, follow: false }`, the pattern is already in `app/admin/(shell)/layout.tsx`); it is the one privacy choice in this project that cannot be undone by shipping a fix, because a search cache outlives the deploy that filled it. And **no unauthenticated route may return an email or an EID** — `leaderboard` carries `id`, `full_name` and `total_points` and nothing else, which `tests/security.test.ts` pins.<br>📌 `/lookup` is identity-based with no account (§2.3), so it re-opens the §6 membership-oracle question that `/attend` settled deliberately — read `docs/attend-confirmation-flow.md` before designing it rather than after. |
...
- `seed.sql` never named `member_field_definitions` or `member_filter_presets`, and `scripts/seed-remote.sh` can only clear what the list names — which is why production still carries a `shirt_size` definition from the phase-4 walkthrough, long after "production IS the seed" was recorded as true. `db reset` hides it locally because it drops the whole database rather than running these deletes.
...
- [ ] 🪤 **Production still carries a stray `shirt_size` field definition** from the phase-4 walkthrough (8 options, directory column, no member holds a value). Phase 9 fixed the *cause* — `seed.sql` now wipes `member_field_definitions` — but **production has not been re-seeded**, so the row is still there.
  - **Trigger:** the term-start reset that `tasks.md` already anticipates. Decide then whether `shirt_size` should simply be *seeded*, since somebody clearly wants the column.
...
- **Stage 7 — Member-facing views** · 3 days · `/leaderboard` and `/lookup`
  - 📌 Consider taking `/admin/audit` with it — see the carried-out items above. It is officer-facing rather than member-facing, so it does not belong to this stage by theme, but it is the cheapest moment to build a screen five Stage 6 verbs are already writing to.
