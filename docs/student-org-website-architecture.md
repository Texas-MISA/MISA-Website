...
| Vercel | ✔ | Possible, but re-importing is easier | Currently the MISA email's personal **Hobby** account (`txmisa-jds-projects`) — Vercel Teams are Pro-only, so there is no org-level scope on the free tier. This is therefore a shared-login handoff, unlike GitHub. Cheap either way: a Vercel project is just the git connection + env vars + domain settings, so a successor imports the repo and pastes the two env vars from `.env.example` in ~10 minutes. |
