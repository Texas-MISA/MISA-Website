...
> - 🔓 **The skills are COMMITTED now** (officer) — this reverses v1.66's gitignore, whose note below is kept for the reasoning. Their licence texts travel with them in `.claude/third-party-licenses/` (impeccable's Apache NOTICE included; Vercel's skill is MIT by its README alone — upstream has no LICENSE file), and the tree was scanned for credentials before committing. 🪤 Installing `ui-ux-pro-max` also dropped SIX unrequested skills into `.claude/skills/` (brand, banner-design, design, design-system, slides, ui-styling); all six were removed, because each is a second opinion on concerns the roster already assigns.
...
> - 📌 **What the axe runs found.** Every public page, `/admin/login`, the 404 and all four portal pages pass on FIRST RENDER at WCAG 2.2 AA — including the three still carrying the `--misa-muted` failure, because that ink appears only in states a first render never shows. So the suite walks those states: `/portal/lookup`'s result fails `definition-list` (a `<p>` note inside a `<dl>` group, `lookup-form.tsx:435`), and **both of `/portal/attend`'s first-timer confirmations fail `color-contrast` on the muted `<dt>` labels on the grey panel — the known failure, caught by a check for the first time.** The attend states run against a short-lived LOCAL event and write no attendance. All three are phase 3's to fix; red on a `legacy` surface is expected.
...
> - 🔍 **Reviewed the same day, and the review found the system's own defects** (officer: "conduct a comprehensive review … implement changes"). Five were bugs: YAML read the SHA `1836e72` — one of this branch's own commits — as the float 1.836e+75; the detector test threw on the detector's exit 2 before consulting its baseline, so a baseline could never work; `npm run test:ui` could not run beside `npm run dev`, because Next 16 locks a project against a second `next dev`; the lookup state tests would have passed on the rate-limit banner; and the rewrite of DESIGN.md's precedence section had DROPPED the ban on impeccable's replace-DESIGN.md path. Four were design weaknesses: the brief lived where impeccable never reads it (it is now impeccable's own surface brief), adopting one concept satisfied "a skill changed something" on its own, evidence and detector hits could be recorded without being used, and freshness would have called every build commit stale. All are fixed. Each checker fix and the detector fix has a test that was shown to FAIL with its fix reverted; the dev-server reuse was run beside a live `npm run dev` (same 39 pass / 3 fail, no lock collision); the state tests now assert the state they reached.
...
>
> Officer instruction, 2026-09-17. Copy only; shipped to production as `ce5bda7`.
>
> - **The label was "This is my first MISA event" and is now "This is my first MISA event, or my first time checking in here."** The matching sentence in the `unmatched` banner moved with it. The box has always meant *"I am not on the roster yet"*, and the old label undersold that: a member who attended before this site existed, or who was added by an officer from a roster import, is on file but had never seen the form — and the label invited them to read "first event" literally and leave the box unticked, which is the one path that writes nothing and re-prompts.
> - 📌 **No behaviour changed, and that is the point of recording it here.** The `firstTime` field name, the decision table in §4.2, the hint-not-an-instruction property, and the confirmation pass are all untouched — ticking the box when you already exist still links you to yourself and never creates a second person. This entry exists because §4.2, §6 and §9 #2 quote the label verbatim as the mechanism's name, so the quotes had to move with the copy or the doc would describe a control the form no longer shows.
> - 🪤 **The `v1.22` entry below still quotes the ORIGINAL label, deliberately.** It records what was decided in July 2026 and is left as written; a changelog rewritten to match today's copy stops being a record. The same applies to `docs/build-log.md`. When the two disagree, the current-behaviour sections (§4.2, §4.3, §6, §9 #2) are the ones that describe the live form.
...
> President. 🔓 **Officer headshots ship for all thirteen**, which
...
- **Member lookup is resolution order; creating a member is a separate, confirmed step** (v1.22). A check-in resolves its member link by trying, in order: `normalized_eid`, then `lower(email)`. Matching on email second is what contains the common failure — someone who typos their EID is recognized by their email and linked to their existing record instead of becoming a duplicate person.
  ...
  What happens when *both* miss depends on one checkbox on the form, "this is my first MISA event, or my first time checking in here":
