...
> ⏭️ **NEXT TASK after phase 1 merges (officer, 2026-09-18): a UI redesign of the portal and every page in it** — the hub, `/portal/attend`, `/portal/leaderboard`, `/portal/lookup`. That is v2 phase 3 in [`frontend-redesign-v2-plan.md`](frontend-redesign-v2-plan.md), widened to the hub. It is presentation only, and it owns the `--misa-muted` AA fix. Phase 1 deliberately built the hub from shared primitives so as not to pre-empt it. 🧰 **It runs through the design toolkit** (`DESIGN.md` §Design toolkit, built 2026-09-18 on `design-toolkit`, stacked on `portal-phase-1`): `/design-brief` for each portal surface — ✅ **done for all four on 2026-09-19**, each with an officer-adopted concept — then the build, then `/design-gate`. The briefs are at `.impeccable/surfaces/route-portal*.md`; the state a build starts from is [`frontend-redesign-v2-plan.md`](frontend-redesign-v2-plan.md) §Phase 3 brief.
