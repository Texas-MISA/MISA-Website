...
- 🔓 **`member_directory` is ONE ROW PER (MEMBER, TERM)** since migration 29 (2026-08-25), and `leaderboard` is defined over it scoped to `current_term()`. Any new aggregate must be scoped to **the row's term**, denominators included — an all-time `events_possible` against a term-scoped `events_attended` understates every rate and still looks plausible. `pending_count` and `last_seen_at` are the only columns that are deliberately all-time, and both must be labelled wherever they render. (§4.4, §4.5)

  - **Membership is derived, never ticked.** A member is on a term's roster if they were marked `present` at one of its events, were granted or docked points in it, have a live payment covering it, or joined during it. `members.joined_at` is NOT NULL, so **every member always has at least one row** — which is what lets a caller treat an empty result as "no such member" rather than as an absence.
  - 🪤 **`.eq("id", …).maybeSingle()` on this view is a PGRST116 waiting for somebody's second semester.** Read the rows and pick the term; a member with no row for the scope is a NORMAL state (they joined earlier and have done nothing since) and renders as zeros, never as "not found".
  - 🪤 **`applyMemberFilter` takes the current term as a REQUIRED argument**, exactly as it takes the field definitions and for the same reason: a caller that forgets it is a compile error rather than a directory that silently returns every member once per term with a count that reads as a member total. There is no honest default — the module cannot know what term it is, and §4.7 forbids it from guessing.
  - ⚠️ **The board can now be EMPTY**, which the old `LEFT JOIN` over every active member made impossible, and an empty board carries no `term` for the heading. `/leaderboard` handles a null term already. **Never add an `rpc("current_term")` fallback for that case** — it is the two-sources-for-one-fact defect migration 21 closed.
  - ⚠️ **The week-one consequence is not a bug:** before a term's first event, its roster is only the members who joined during it or have already paid dues covering it. The directory's empty state names the four ways onto the roster instead of saying "no members".

- ⚠️ **`members.active` was DROPPED by migration 29 and is irreversible.** A hand-ticked boolean was the only answer to "is this person still in the club", and nothing kept it current: it defaulted to true, the roster import could set it, and after that it drifted. It is not replaced by a better flag — it is replaced by the derivation above. Removing it reached well past the directory, and each of these went with it rather than being reworded: the attendance and dues candidate scans and their −20 inactive penalty; the `member_inactive` resolution warning (whose stated justification was that `leaderboard` filtered on the column); `fetchMemberOptions`' active-only predicate **and** its `includeId` escape hatch, which existed only to keep a deactivated member in the picker; `parseActive` and the roster import's `Active` column; the export catalogue's **Active** field, now **Term**; and the inactive badges on the directory, dues, points and import screens. ⚠️ **The candidate scans now read the WHOLE roster**, so `MEMBER_SCAN_LIMIT` (400) bites sooner than it used to.
...
- 🔴 **`/lookup`'s gate is the EID ALONE as of 2026-08-25 (officer's instruction), and dues status was kept on the page.** This REVERSES the invariant below, which is preserved after it because the reasoning is what the change spent.

  What is true now: `findMemberByEid` in `lib/lookup.ts` resolves on `normalized_eid` in one query and nothing else. A UT EID is derived from a person's initials rather than issued at random — the same property `lib/attendance.ts` relies on when it refuses to corroborate a near-miss at edit distance 2 — so guessing one is cheap, and anyone who guesses one sees that member's name, point total, attendance history and dues status. `LOOKUP_RATE_LIMIT_MAX` is the only remaining cost, and it is a *room number* sized against the largest venue the org books, not a security parameter. It should not be talked down on abuse grounds and it is no longer a second line of defence behind a strong first one.

  **Still binding, and still pinned by a source assertion:** ONE query; no `.or()` widening the match to another column; no borrowing of `/attend`'s ordered fallback; and one `unmatched` outcome for every miss (below), which now carries more weight than when it was written.

- 🔓 ~~**`/lookup`'s gate is a conjunction expressed in ONE query, never `/attend`'s ordered fallback.**~~ **(Superseded 2026-08-25 — kept for the argument.)** `findMember` in `lib/checkin.ts` resolves EID *then* email because a check-in must forgive one mistyped field; `findMemberByBoth` in `lib/lookup.ts` required both to name the same row, because this is the only surface that reveals **dues status** and §6 accepted that exposure *solely* on the strength of the narrower gate. Two lookups, or an `.or()`, silently reduces it to EID-alone. The email predicate went through `escapeIlike` so a `%` could not become a wildcard past half the gate.
...
- 🪤 **The site header's nav cannot grow without measuring at 1280.** The wordmark is absolutely centred and wins the z-order; an overflowing item disappears rather than wrapping. 🔓 **RE-MEASURED 2026-09-18: 342px clearance left, 450px right** (was 342/295 on 2026-08-23, and 285/312 before that). Left group 225px; the right side is ONE navy MEMBER PORTAL button, 117px (was 272 — Leaderboard, My Attendance and the Check In button, replaced by the member portal); wordmark 82px, 32px gutter. **The left is the tighter side again.** 🪤 On a phone that button sits beside the centred wordmark alone: at the shared `sm` padding it was 2.4px clear at 360 and 25px UNDER the mark at 320, so it takes `px-3` below `sm` (10.4px clear at 360) and stacks onto two lines below 360px (33px clear at 320). Relisting `/projects` spends part of the left; any sixth item needs a fresh measurement. (See `DESIGN.md` §Nav clearance.)
