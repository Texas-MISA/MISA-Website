...
| **7 — Records, merge and handoff** | `DESIGN.md`'s surface table and `docs/design/surfaces.json` agree; ✅ `PageHero`'s "NINE pages render this" corrected to five — **done 2026-09-20**, and it became TRUE rather than merely corrected: the four portal surfaces left the hero when the officer removed the navy header, so a change to it now reaches the five content pages and nothing else (`chevron-section.tsx`, DESIGN.md, `docs/layout.md`); `tasks.md` and `build-log.md` record what the phase found; the officer's gate. | Merged (**never squashed** — receipts name commits), with `portal-phase-1` merged first. |
